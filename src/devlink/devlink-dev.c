...
static int devlink_dev_genl_eswitch_set(
        ...
        if (r < 0)
                return log_devlink_debug_errno(devlink, r, "Could not send dev eswitch set message: %m");

        r = sd_netlink_message_get_errno(rep);
        if (r < 0)
                return log_devlink_debug_errno(devlink, r, "Dev eswitch could not be set: %m");
...
static int devlink_dev_genl_eswitch_get(
        ...
        r = sd_netlink_call(devlink->manager->genl, req, 0, &rep);
        if (r < 0)
                return log_devlink_debug_errno(devlink, r, "Could not call dev eswitch get message: %m");

        r = sd_netlink_message_get_errno(rep);
        if (r < 0)
                return log_devlink_debug_errno(devlink, r, "Dev eswitch could not be get: %m");
...
static int devlink_dev_genl_cmd_new_msg_process(
        ...
        if (dev->eswitch_mode != _DEVLINK_DEV_ESWITCH_MODE_INVALID) {
                ...
                if ((uint16_t) dev->eswitch_mode != current_eswitch_mode) {
                        ...
                        log_devlink_debug(devlink, "Dev eswitch mode set skipped");
                }
        }
...
const DevlinkVTable devlink_dev_vtable = {
        ...
        .sections = DEVLINK_COMMON_SECTIONS "Eswitch\0",
