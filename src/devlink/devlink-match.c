...
#define devlink_match_vtable_size array_size(devlink_match_vtable)

void devlink_match_init(DevlinkMatch *match) {
        unsigned int i;

        for (i = 0; i < devlink_match_vtable_size; i++) {
                if (devlink_match_vtable[i]->init)
                        devlink_match_vtable[i]->init(match);
        }
}
...
bool devlink_match_check(const DevlinkMatch *match, DevlinkMatchSet matchset) {
        ...
        for (i = 0; i < devlink_match_vtable_size; i++) {
                if (!devlink_matchset_bit_check(matchset, i))
                        continue;
                ret = devlink_match_vtable[i]->check(match);
                if (!ret)
                        return ret;
        }
        return true;
