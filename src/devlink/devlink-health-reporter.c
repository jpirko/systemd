...
static int devlink_health_reporter_grace_period_update(
        ...
        log_devlink_debug(devlink, "Grace period current value \"%u\", desired value \"%u\".", current_val, val);

        if (current_val == val)
                return 0;

        log_devlink_info(devlink, "Grace period value to be set: \"%u\".", val);
...
static int devlink_health_reporter_genl_cmd_get_msg_process(
        ...
        if (r < 0)
                return log_devlink_debug_errno(devlink, r, "Could not send health reporter set message: %m");

        r = sd_netlink_message_get_errno(rep);
        if (r < 0)
                return log_devlink_debug_errno(devlink, r, "Health reporter could not be set: %m");
...
static const DevlinkMatchSet devlink_health_reporter_matchsets[] = {
        DEVLINK_MATCH_BIT_PORT_CACHED_IFNAME | DEVLINK_MATCH_BIT_HEALTH_REPORTER,
        DEVLINK_MATCH_BIT_DEV | DEVLINK_MATCH_BIT_PORT_INDEX | DEVLINK_MATCH_BIT_HEALTH_REPORTER,
        DEVLINK_MATCH_BIT_DEV | DEVLINK_MATCH_BIT_HEALTH_REPORTER,
        0,
};

static const DevlinkMonitorCommand devlink_health_reporter_commands[] = {
        { DEVLINK_CMD_HEALTH_REPORTER_GET, devlink_health_reporter_genl_cmd_get_msg_process },
};

const DevlinkVTable devlink_health_reporter_vtable = {
        .object_size = sizeof(DevlinkHealthReporter),
        .sections = DEVLINK_COMMON_SECTIONS "HealthReporterMatch\0PortMatch\0HealthReporter\0",
        ...
        .genl_enumerate_cmd = DEVLINK_CMD_HEALTH_REPORTER_GET,
